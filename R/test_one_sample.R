...
#' @return A `testflow` object with class `testflow_one_sample`. The object is
...
test_one_sample <- function(data, outcome, mu = 0, alternative = c("two.sided", "less", "greater"), alpha = 0.05, plot = TRUE, na.rm = TRUE) {
  ...
  xval <- df[[outcome_nm]]
  symmetry <- assumption_check("Symmetry of deviations", ifelse(recommendation == "One-sample t-test", "not checked", "acceptable"), ifelse(recommendation == "One-sample t-test", "Normality made the symmetry check unnecessary.", "Wilcoxon signed-rank relies on approximate symmetry."), method = "Signed-rank teaching note")
